...
def _tela(stdscr, dados: list, destaque: tuple | None):
    curses.curs_set(0)
    _init_cores()

    book_idx  = destaque[0] if destaque else 0
    cap_idx   = destaque[1] if destaque else 0
    ver_scroll  = 0
    book_scroll = 0
    ...
    while True:
        ...
        for i in range(h - 2):
            ...
            if idx >= len(dados):
                break
            nome = dados[idx]["name"]
            nome_trunc = nome[: LIVRO_W - 3]
            if idx == book_idx:
                attr   = curses.color_pair(COR_LIVRO) | curses.A_BOLD
                marker = "►"
            else:
                attr   = curses.color_pair(COR_NORMAL)
                marker = " "
            try:
                stdscr.addstr(i + 1, 0, f"{marker} {nome_trunc}", attr)
            except curses.error:
                pass

        # ── Cabeçalho do painel de versículos ─────────────────────────────────
        ...
        # Garante que o versículo destacado esteja visível ao abrir
        if destaque and destaque[0] == book_idx and destaque[1] == cap_idx:
            alvo = destaque[2]
            primeira_linha_alvo = next(
                (li for li, (vi, _) in enumerate(linhas) if vi == alvo), 0
            )
            area = h - 4
            if primeira_linha_alvo < ver_scroll:
                ver_scroll = primeira_linha_alvo
            elif primeira_linha_alvo >= ver_scroll + area:
                ver_scroll = max(0, primeira_linha_alvo - area // 2)
            destaque = None   # só centraliza na primeira vez

        area_versos = h - 3   # linhas disponíveis para versículos
        for row in range(area_versos):
            li = row + ver_scroll
            if li >= len(linhas):
                break
            vi, linha = linhas[li]
            ...
            eh_dest = (
                destaque is None   # já foi consumido acima
                # mas precisamos manter o highlight permanente: usamos book/cap/ver salvos
            )
            # Highlight permanente: compara com índices originais salvos em _destaque_orig
            attr = curses.color_pair(COR_VERSO)
            try:
        ...
        # ── Rodapé ────────────────────────────────────────────────────────────
        rodape = f" Livro {book_idx + 1}/{len(dados)}  Cap {cap_idx + 1}/{len(livro['chapters'])}  " \
                 f"Linha {ver_scroll + 1}/{max(1, len(linhas))}"
        try:
        ...
        if key in (ord('q'), ord('Q'), 27):
            ...
            ver_scroll = min(max(0, len(linhas) - area_versos), ver_scroll + 1)

        elif key == curses.KEY_PPAGE:
            ver_scroll = max(0, ver_scroll - area_versos)

        elif key == curses.KEY_NPAGE:
            ver_scroll = min(max(0, len(linhas) - area_versos), ver_scroll + area_versos)

        elif key == curses.KEY_HOME:
            ver_scroll = 0

        elif key == curses.KEY_END:
            ver_scroll = max(0, len(linhas) - area_versos)
        ...
        # Ajusta scroll do painel de livros
...
def _tela_com_destaque(stdscr, dados: list, destaque_orig: tuple | None):
...
def run(dados: list, referencia: str | None = None):
    ...
    curses.wrapper(_tela_com_destaque, dados, destaque)
